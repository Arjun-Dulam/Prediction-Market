...
uint32_t OrderGenerator::generate_quantity() {
  // Formula -> x = [(x_max^(pow) - x_min^(pow)) * y_unif + x_min^(pow)] ^ (1 /
  // pow)
...
bool OrderGenerator::should_cancel() { return (cancel_dist(rng)); }

Order OrderGenerator::generate_order() {
  Order new_order(generate_price(), generate_quantity(), generate_side());
