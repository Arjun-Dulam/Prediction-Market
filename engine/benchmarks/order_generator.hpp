...
class OrderGenerator {
 private:
  ...
  /**
   * Normal Distribution accurately models the clustering of prices near the
   * true price with exponentially rarer instances of prices farther away from
   * the true price.
   */
  std::normal_distribution<double> price_dist;

  /**
   * Used as input for the power law function, which we use to model the
   * differing quantities of orders. Vast majority of orders are retail
   * investors with small quantities while a small subset of orders is
   * institutional orders with quantities in the thousands.
   */
